...
def evidential_kd_loss(logits_student_in, logits_teacher_in, logit_stand, T):
...
class EKDIN(Distiller):
    """Distilling the Knowledge in a Neural Network"""

    def __init__(self, student, teacher, cfg):
        super(EKDIN, self).__init__(student, teacher)
        ...
        # self.ce_lamb_S = nn.Parameter(torch.tensor(0.2, dtype=torch.float32))
        ...
        self.ce_lamb_S = torch.tensor(0.0)
        self.ce_lamb_T = torch.tensor(0.0)
        self.lamb_2nd_order_S = nn.Parameter(torch.tensor(69.0, dtype=torch.float32)) #second-order lamb
        self.lamb_2nd_order_T = nn.Parameter(torch.tensor(70.0, dtype=torch.float32)) #second-order lamb
    ...
    def forward_train(self, image, target, **kwargs):
        ...
        if self.expectation:
            ...
            loss_kd = self.kd_loss_weight * evidential_kd_loss(
                    logits_student, logits_teacher, self.logit_stand, self.temperature
            )
        ...
        loss_ekd = self.ekd_loss_weight  * compute_ekd_loss(alpha_teacher, alpha_student)
