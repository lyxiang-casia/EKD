...
def get_evidence(logits, efunction):
        if efunction == "relu":
            ...
            logits_clamped = torch.clamp(logits, max=80)
            evidence = torch.exp(logits_clamped)
        # elif efunction == "sigmoid":
        #     evidence = F.sigmoid(logits)
        elif efunction == "softmax":
            evidence = F.softmax(logits, dim=1)
        elif efunction == "log1p":
            evidence = torch.log1p(logits)
        else:
...
class Teacher(nn.Module):
    ...
    def forward_train(self, image, target, **kwargs):
        ...
        # compute loss_ce
        evidence = get_evidence(logits_student, self.efunction)
        alpha = evidence + get_evidence(self.lamb, self.efunction)
        ...
        if self.KL:
            ...
            loss_kl = np.minimum(1.0, epoch / 10.) * compute_kl_loss(kl_alpha, self.lamb)
