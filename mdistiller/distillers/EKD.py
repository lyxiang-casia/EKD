...
def compute_ce_loss(logits_student, target):
...
def dkd_softmax_loss(logits_student_in, logits_teacher_in, target, alpha, beta, temperature, logit_stand):
...
class EKD(Distiller):
    ...
    def forward_train(self, image, target, **kwargs):
        ...
        # compute loss_ce
        loss_ce = self.ce_loss_weight * compute_ce_loss(logits_student, target)
        ...
        # loss_kd =  * projected_kd_loss(alpha_student, alpha_teacher)
        # loss_dkd = min(kwargs["epoch"] / self.warmup, 1.0) * 10 * dkd_loss(
        #     alpha_student,
        #     alpha_teacher,
        #     target,
        #     1.0,
