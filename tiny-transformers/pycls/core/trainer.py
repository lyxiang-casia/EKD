...
from pycls.core.net import SoftCrossEntropyLoss, EvidentialCELoss
...
def train_epoch(loader, model, ema, loss_fun, optimizer, scheduler, scaler, meter, cur_epoch):
    ...
    for cur_iter, (inputs, labels, offline_features) in enumerate(loader):
        ...
        with amp.autocast(enabled=cfg.TRAIN.MIXED_PRECISION):
            preds = model(inputs)
            if isinstance(loss_fun, EvidentialCELoss):
                loss_cls = loss_fun(preds, labels_one_hot, net.unwrap_model(model).lamb)
            else:
                loss_cls = loss_fun(preds, labels_one_hot)
            loss, loss_inter, loss_logit, loss_ekd = loss_cls, inputs.new_tensor(0.0), inputs.new_tensor(0.0), inputs.new_tensor(0.0)
            if hasattr(net.unwrap_model(model), 'guidance_loss'):
                loss_inter, loss_logit, loss_ekd = net.unwrap_model(model).guidance_loss(inputs, offline_features, cur_epoch +1)
                if cfg.DISTILLATION.ENABLE_LOGIT:
                    loss_cls = loss_cls * cfg.DISTILLATION.CLS_WEIGHT
        ...
        loss_cls, loss_inter, loss_logit, loss, top1_err, top5_err, lamb, loss_ekd = dist.scaled_all_reduce([loss_cls, loss_inter, loss_logit, loss, top1_err, top5_err, net.unwrap_model(model).lamb, loss_ekd])
        loss_cls, loss_inter, loss_logit, loss, top1_err, top5_err, lamb, loss_ekd = loss_cls.item(), loss_inter.item(), loss_logit.item(), loss.item(), top1_err.item(), top5_err.item(), lamb.item(), loss_ekd.item()
        meter.iter_toc()
        mb_size = inputs.size(0) * cfg.NUM_GPUS
        meter.update_stats(top1_err, top5_err, loss_cls, loss_inter, loss_logit, loss, lr, mb_size, lamb, loss_ekd)
