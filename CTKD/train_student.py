...
from distiller_zoo import PKT, DistillKL, DKDloss, Similarity, VIDLoss, DistillKL_logit_stand, EKD, evidentialCE, evidentialDiv
...
def parse_option():
    ...
    # distillation
    parser.add_argument('--distill', type=str, default='kd', choices=['ekd','kd', 'similarity', 'vid',
                                                                       'pkt', 'crd', 'dkd', 'srrl'])
    ...
    parser.add_argument('--lamb', type=float, default=-1.22)
...
def load_teacher(teacher, n_cls, gpu=None, opt=None):
    print('==> loading teacher model')
    net, pretrain_model_path = cifar_model_dict[teacher]
    model_teacher = net(num_classes=n_cls)
    ...
    if opt.dataset == 'cifar100':
        model_teacher.load_state_dict(torch.load(pretrain_model_path, map_location=map_location)['model'])
...
def main():
    ...
    if opt.wandb:
        wandb.init(project=opt.project, name=opt.experiments_name)
    # ASSIGN CUDA_ID
...
def main_worker(gpu, ngpus_per_node, opt):
    ...
    # model
    model_t = load_teacher(opt.model_t, n_cls, opt.gpu, opt)
    ...
    if opt.distill == 'ekd':
        criterion_cls = evidentialCE()
        criterion_div = evidentialDiv()
    else: 
        criterion_cls = nn.CrossEntropyLoss()
        criterion_div = DistillKL()
    if opt.distill == 'kd':
        criterion_kd = DistillKL()
    elif opt.distill == 'ekd':
        criterion_kd = EKD()
    ...
    for epoch in range(1, opt.epochs + 1):
        ...
        train_acc, train_acc_top5, train_loss, loss_ce, loss_kl, loss_kd, temp = train(epoch, train_loader, module_list, mlp, decay_value, criterion_list, optimizer, opt)
