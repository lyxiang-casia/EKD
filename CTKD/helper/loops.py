...
def train_distill(epoch, train_loader, module_list, mlp_net, cos_value, criterion_list, optimizer, opt):
    ...
    for idx, data in enumerate(train_loader):
        ...
        # cls + kl div
        if opt.distill == 'ekd':
            loss_cls = criterion_cls(logit_s, target, opt.lamb)
            loss_div = criterion_div(logit_s, logit_t, temp, opt.lamb)
        else:
        ...
        if opt.distill == 'kd':
            ...
            loss_kd = criterion_kd(logit_s, logit_t, opt.lamb, epoch, opt.warmup)
        else:
        ...
        loss_ce.update(loss_cls.item(), input.size(0))
        loss_ekd.update(loss_kd.item(), input.size(0))
        losses.update(loss.item(), input.size(0))
        ...
        if opt.wandb:
            import wandb
            wandb.log({"temp": temp})
    return top1.avg, top5.avg, losses.avg, loss_ce.avg, loss_kl.avg, loss_ekd.avg, temp
...
def validate(val_loader, model, criterion, opt):
    ...
    with torch.no_grad():
        ...
        for idx, batch_data in enumerate(val_loader):
            ...
            loss = criterion(output, target, opt.lamb)
            losses.update(loss.item(), input.size(0))
